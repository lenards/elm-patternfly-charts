...
var $author$project$Main$footerView = A2(
	...
	_List_fromArray(
		[
			$elm$html$Html$text('PatternFly v6 design system · terezka/elm-charts · Elm 0.19.1')
		]));
...
var $author$project$Main$header = A2(
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$html$Html$text('PF v6 Charts · elm-charts experiment')
				])),
			A2(
			$elm$html$Html$p,
			_List_fromArray(
			...
			_List_fromArray(
				[
					$elm$html$Html$text('terezka/elm-charts · PatternFly v6 design tokens')
				]))
		]));
				...
				case 'AreaChartView':
					return _List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text('Area charts show trends over continuous time. The fill (opacity 0.15) emphasizes the magnitude of the metric while the 2px line preserves readability. PF v6 uses monotone interpolation for smooth server metric curves.')
								]))
						]);
				case 'BarChartView':
					return _List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text('Bar charts compare discrete categories. PF v6 multi-ordered color scale maximizes contrast between adjacent bars. Rounded tops (2px radius) match the contemporary PatternFly aesthetic.')
								]))
						]);
				default:
					return _List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text('Stacked area charts show part-to-whole relationships over time. PF v6 uses the multi-unordered scale so each series remains visually distinct across the full time axis.')
								]))
						]);
			}
		}());
...
var $author$project$PF$Colors$axisLine = '#c7c7c7';
...
var $author$project$Internal$Svg$defaultDot = {border: '', borderOpacity: 1, borderWidth: 0, color: $author$project$Internal$Helpers$pink, hideOverflow: false, highlight: 0, highlightColor: '', highlightWidth: 5, opacity: 1, shape: $elm$core$Maybe$Nothing, size: 6};
...
var $author$project$Internal$Coordinates$scaleSVGX = F2(
	function (plane, value) {
		var range_ = $author$project$Internal$Coordinates$range(plane.x);
		return ((plane.x.flip ? (range_ - value) : value) * $author$project$Internal$Coordinates$innerWidth(plane)) / range_;
	});
...
var $author$project$Internal$Coordinates$scaleSVGY = F2(
	function (plane, value) {
		var range_ = $author$project$Internal$Coordinates$range(plane.y);
		return ((plane.y.flip ? (range_ - value) : value) * $author$project$Internal$Coordinates$innerHeight(plane)) / range_;
	});
	...
	function (plane, toX, toY, config, datum_) {
		...
		var styleAttrs = _List_fromArray(
			[
				...
				config.hideOverflow ? $author$project$Internal$Svg$withinChartArea(plane) : $elm$svg$Svg$Attributes$class('')
			]);
		var showDot = A3($author$project$Internal$Svg$isWithinPlane, plane, xOrg, yOrg) || config.hideOverflow;
...
var $author$project$Chart$addIndexes = F2(
...
var $author$project$Internal$Coordinates$toCartesianX = F2(
	function (plane, value) {
		return plane.x.flip ? (($author$project$Internal$Coordinates$range(plane.x) - A2($author$project$Internal$Coordinates$scaleCartesianX, plane, value - plane.x.marginMin)) + plane.x.min) : (A2($author$project$Internal$Coordinates$scaleCartesianX, plane, value - plane.x.marginMin) + plane.x.min);
	});
var $author$project$Internal$Coordinates$toCartesianY = F2(
	function (plane, value) {
		return plane.y.flip ? (A2($author$project$Internal$Coordinates$scaleCartesianY, plane, value - plane.y.marginMin) + plane.y.min) : (($author$project$Internal$Coordinates$range(plane.y) - A2($author$project$Internal$Coordinates$scaleCartesianY, plane, value - plane.y.marginMin)) + plane.y.min);
	});
...
var $author$project$Internal$Svg$decoder = F2(
	function (plane, toMsg) {
		...
		return A4(
			...
			$K_Adam$elm_dom$DOM$target($author$project$Internal$Svg$decodePosition));
	});
...
var $author$project$Chart$chartAndPlane = F2(
...
var $author$project$Chart$chart = F2(
	function (edits, unindexedElements) {
		return A2($author$project$Chart$chartAndPlane, edits, unindexedElements).a;
	});
var $author$project$AreaChart$cpuData = _List_fromArray(
	[
		{cpuPercent: 34, minuteOffset: 0},
		{cpuPercent: 42, minuteOffset: 10},
		{cpuPercent: 58, minuteOffset: 20},
		{cpuPercent: 71, minuteOffset: 30},
		{cpuPercent: 65, minuteOffset: 40},
		{cpuPercent: 53, minuteOffset: 50},
		{cpuPercent: 48, minuteOffset: 60},
		{cpuPercent: 62, minuteOffset: 70},
		{cpuPercent: 78, minuteOffset: 80},
		{cpuPercent: 85, minuteOffset: 90},
		{cpuPercent: 74, minuteOffset: 100},
		{cpuPercent: 61, minuteOffset: 110},
		{cpuPercent: 55, minuteOffset: 120}
	]);
...
var $author$project$Internal$Item$render = function (_v0) {
	var item = _v0.b;
...
var $author$project$Internal$Coordinates$Point = F2(
	function (x, y) {
		return {x: x, y: y};
	});
...
var $author$project$Internal$Item$getPosition = function (_v0) {
	var item = _v0.b;
...
var $author$project$Chart$seriesMap = F4(
	function (mapData, toX, properties, data) {
		return $author$project$Chart$Indexed(
			F2(
				function (_v0, index) {
					var legends = A2($author$project$Internal$Legend$toDotLegends, index, properties);
					var _v1 = A4($author$project$Internal$Produce$toDotSeries, index, toX, properties, data);
...
var $author$project$Chart$Attributes$attrs = function (v) {
	return $author$project$Internal$Helpers$Attribute(
		function (config) {
			return _Utils_update(
				config,
...
var $author$project$Chart$xAxis = function (edits) {
	var config = A2(
		$author$project$Internal$Helpers$apply,
		edits,
		{arrow: true, attrs: _List_Nil, color: '', limits: _List_Nil, pinned: $author$project$Chart$Attributes$zero, width: 1});
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$author$project$Chart$Attributes$color(config.color),
								p.x.flip ? $author$project$Chart$Attributes$rotate(180) : $author$project$Chart$Attributes$rotate(0)
							]),
						{
							x: xLimit.max,
							y: config.pinned(p.y)
...
var $author$project$Internal$Svg$defaultLabel = {anchor: $elm$core$Maybe$Nothing, attrs: _List_Nil, border: 'white', borderWidth: 0, color: '#808BAB', ellipsis: $elm$core$Maybe$Nothing, fontSize: $elm$core$Maybe$Nothing, hideOverflow: false, rotate: 0, uppercase: false, xOff: 0, yOff: 0};
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$author$project$Chart$Attributes$color(config.color),
								p.y.flip ? $author$project$Chart$Attributes$rotate(90) : $author$project$Chart$Attributes$rotate(-90)
							]),
						{
							x: config.pinned(p.x),
							y: yLimit.max
...
var $author$project$AreaChart$chart = A2(
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$author$project$Chart$Attributes$color($author$project$PF$Colors$gridLine),
					$author$project$Chart$Attributes$width(1)
				])),
			$author$project$Chart$xAxis(
			_List_fromArray(
				[
					$author$project$Chart$Attributes$color($author$project$PF$Colors$axisLine)
				])),
			$author$project$Chart$xLabels(
			_List_fromArray(
				[
					$author$project$Chart$Attributes$color($author$project$PF$Colors$labelText),
					$author$project$Chart$Attributes$fontSize(12),
					$author$project$Chart$Attributes$format(
					function (t) {
						return $elm$core$String$fromFloat(t) + 'm';
					}),
					$author$project$Chart$Attributes$amount(7)
				])),
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$author$project$Chart$Attributes$color($author$project$PF$Colors$primary),
							$author$project$Chart$Attributes$width(2),
							$author$project$Chart$Attributes$monotone,
							$author$project$Chart$Attributes$opacity(0.15)
						]),
					_List_Nil)
				]),
			$author$project$AreaChart$cpuData)
		]));
...
var $author$project$AreaChart$view = A2(
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$html$Html$text('CPU Utilization')
				])),
			A2(
			$elm$html$Html$p,
			_List_fromArray(
			...
			_List_fromArray(
				[
					$elm$html$Html$text('compute-node-01 · last 2 hours')
				])),
...
var $author$project$Chart$barsMap = F4(
	function (mapData, edits, properties, data) {
		return $author$project$Chart$Indexed(
			F2(
				function (_v0, index) {
					var legends = A3($author$project$Internal$Legend$toBarLegends, index, edits, properties);
					var barsConfig = A2($author$project$Internal$Helpers$apply, edits, $author$project$Internal$Produce$defaultBars);
					var _v1 = A4($author$project$Internal$Produce$toBarSeries, index, edits, properties, data);
					var newElementIndex = _v1.a;
					var limits = _v1.b;
					var items = _v1.c;
					var toItems = F2(
...
var $author$project$BarChart$hostLabel = function (n) {
	return 'node-0' + $elm$core$String$fromInt(
		$elm$core$Basics$round(n));
};
var $author$project$BarChart$memoryData = _List_fromArray(
	[
		{cached: 18, free: 68, host: 1, used: 42},
		{cached: 12, free: 29, host: 2, used: 87},
		{cached: 22, free: 51, host: 3, used: 55},
		{cached: 8, free: 89, host: 4, used: 31},
		{cached: 15, free: 39, host: 5, used: 74},
		{cached: 19, free: 48, host: 6, used: 61}
	]);
...
var $author$project$BarChart$pfColor = function (idx) {
	return A2(
		...
		$elm$core$List$head(
			A2($elm$core$List$drop, idx, $author$project$PF$Colors$multiOrdered)));
};
...
var $author$project$BarChart$chart = A2(
	...
	_List_fromArray(
		[
			$author$project$Chart$Attributes$width(620),
			$author$project$Chart$Attributes$height(300),
			$author$project$Chart$Attributes$margin(
			{bottom: 50, left: 60, right: 20, top: 10}),
		...
		[
			...
			_List_fromArray(
				[
					$author$project$Chart$Attributes$color($author$project$PF$Colors$labelText),
					$author$project$Chart$Attributes$fontSize(12),
					$author$project$Chart$Attributes$format($author$project$BarChart$hostLabel)
				])),
			$author$project$Chart$yAxis(
			_List_fromArray(
				[
			...
			_List_fromArray(
				[
					...
					$author$project$Chart$Attributes$format(
					function (v) {
						return $elm$core$String$fromFloat(v) + ' GiB';
					}),
					$author$project$Chart$Attributes$amount(6)
				])),
			A3(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$author$project$Chart$Attributes$color(
							$author$project$BarChart$pfColor(0)),
							$author$project$Chart$Attributes$roundTop(2)
						])),
					A2(
					$author$project$Chart$bar,
					function ($) {
					...
					_List_fromArray(
						[
							$author$project$Chart$Attributes$color(
							$author$project$BarChart$pfColor(1)),
							$author$project$Chart$Attributes$roundTop(2)
						])),
					A2(
					$author$project$Chart$bar,
					...
					_List_fromArray(
						[
							$author$project$Chart$Attributes$color(
							$author$project$BarChart$pfColor(2)),
							$author$project$Chart$Attributes$roundTop(2)
						]))
				]),
			$author$project$BarChart$memoryData)
		]));
var $author$project$BarChart$legendItem = F2(
	function (_v0, _v1) {
		var label = _v1.a;
		var colorIdx = _v1.b;
		return A2(
				...
				[
					...
					_List_fromArray(
						[
							...
							$elm$html$Html$Attributes$style,
							'background-color',
							$author$project$BarChart$pfColor(colorIdx))
						]),
					_List_Nil),
					$elm$html$Html$text(label)
				]));
	});
var $author$project$BarChart$legend = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
			A2($elm$html$Html$Attributes$style, 'display', 'flex'),
	...
	A2(
		$elm$core$List$indexedMap,
		$author$project$BarChart$legendItem,
		_List_fromArray(
			[
				_Utils_Tuple2('Used', 0),
				_Utils_Tuple2('Cached', 1),
				_Utils_Tuple2('Free', 2)
			])));
var $author$project$BarChart$view = A2(
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$html$Html$text('Memory Usage by Host')
				])),
			A2(
			$elm$html$Html$p,
			_List_fromArray(
			...
			_List_fromArray(
				[
					$elm$html$Html$text('GiB — all compute nodes')
				])),
			$author$project$BarChart$chart,
			$author$project$BarChart$legend
		]));
var $author$project$StackedAreaChart$bandwidthData = _List_fromArray(
	[
...
var $author$project$PF$Colors$multiUnordered = _List_fromArray(
	['#0066cc', '#63993d', '#37a3a3', '#dca614', '#f5921b']);
var $author$project$StackedAreaChart$pfUnorderedColor = function (idx) {
	return A2(
		$elm$core$Maybe$withDefault,
		$author$project$PF$Colors$primary,
		$elm$core$List$head(
			A2($elm$core$List$drop, idx, $author$project$PF$Colors$multiUnordered)));
};
var $author$project$StackedAreaChart$chart = A2(
	$author$project$Chart$chart,
	_List_fromArray(
		[
			$author$project$Chart$Attributes$width(620),
			$author$project$Chart$Attributes$height(300),
			$author$project$Chart$Attributes$margin(
			{bottom: 40, left: 60, right: 20, top: 10}),
			$author$project$Chart$Attributes$domain(
			_List_fromArray(
				[
					A2($author$project$Chart$Attributes$lowest, 0, $author$project$Chart$Attributes$exactly)
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$author$project$Chart$Attributes$color(
							$author$project$StackedAreaChart$pfUnorderedColor(0)),
							$author$project$Chart$Attributes$width(2),
							$author$project$Chart$Attributes$monotone,
							$author$project$Chart$Attributes$opacity(0.12)
						]),
					_List_Nil),
					A3(
					$author$project$Chart$interpolated,
					...
					_List_fromArray(
						[
							$author$project$Chart$Attributes$color(
							$author$project$StackedAreaChart$pfUnorderedColor(1)),
							$author$project$Chart$Attributes$width(2),
							$author$project$Chart$Attributes$monotone,
							$author$project$Chart$Attributes$opacity(0.12)
						]),
					...
					_List_fromArray(
						[
							$author$project$Chart$Attributes$color(
							$author$project$StackedAreaChart$pfUnorderedColor(2)),
							$author$project$Chart$Attributes$width(2),
							$author$project$Chart$Attributes$monotone,
							$author$project$Chart$Attributes$opacity(0.12)
						]),
			...
			$author$project$StackedAreaChart$bandwidthData)
		]));
var $author$project$StackedAreaChart$legendItem = function (_v0) {
	var label = _v0.a;
	var colorIdx = _v0.b;
	return A2(
		$elm$html$Html$div,
			...
			[
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$style,
						'background-color',
						$author$project$StackedAreaChart$pfUnorderedColor(colorIdx))
					]),
				_List_Nil),
				$elm$html$Html$text(label)
			]));
};
var $author$project$StackedAreaChart$legend = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
			A2($elm$html$Html$Attributes$style, 'display', 'flex'),
	...
	A2(
		...
		F2(
			function (_v0, item) {
				return $author$project$StackedAreaChart$legendItem(item);
			}),
		_List_fromArray(
			[
				_Utils_Tuple2('project-alpha', 0),
				_Utils_Tuple2('project-beta', 1),
				_Utils_Tuple2('project-gamma', 2)
			])));
var $author$project$StackedAreaChart$view = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
			A2($elm$html$Html$Attributes$style, 'font-family', '\'Red Hat Text\', Helvetica, Arial, sans-serif'),
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$html$Html$text('Outbound Bandwidth')
				])),
			A2(
			$elm$html$Html$p,
			_List_fromArray(
			...
			_List_fromArray(
				[
					$elm$html$Html$text('Mbps per project · last 90 minutes')
				])),
			$author$project$StackedAreaChart$chart,
			$author$project$StackedAreaChart$legend
		]));
				...
				_List_fromArray(
					[
						...
						switch (_v0.$) {
							...
							default:
								return $author$project$StackedAreaChart$view;
						}
					}()
					])),
...
var $author$project$Main$StackedAreaView = {$: 'StackedAreaView'};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A3($author$project$Main$navTab, 'Stacked Area', $author$project$Main$StackedAreaView, active)
			]));
};
