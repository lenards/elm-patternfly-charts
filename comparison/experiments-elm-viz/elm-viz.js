...
var $author$project$Main$AreaChartView = {$: 'AreaChartView'};
var $elm$core$Platform$Cmd$batch = _Platform_batch;
var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{active: $author$project$Main$AreaChartView},
...
var $author$project$Main$footerView = A2(
	...
	_List_fromArray(
		[
			A2($elm$html$Html$Attributes$style, 'padding', '24px 32px'),
			A2($elm$html$Html$Attributes$style, 'font-size', '12px'),
			A2($elm$html$Html$Attributes$style, 'color', '#8c8c8c'),
			A2($elm$html$Html$Attributes$style, 'border-top', '1px solid #c7c7c7'),
			A2($elm$html$Html$Attributes$style, 'margin-top', '32px')
		]),
	_List_fromArray(
		[
			$elm$html$Html$text('PatternFly v6 design system · gampleman/elm-visualization · Elm 0.19.1')
		]));
...
var $author$project$Main$header = A2(
	...
	_List_fromArray(
		[
			A2($elm$html$Html$Attributes$style, 'background', '#1f1f1f'),
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$html$Html$text('PF v6 Charts · elm-visualization experiment')
				])),
			A2(
			$elm$html$Html$p,
			_List_fromArray(
				[
					...
					A2($elm$html$Html$Attributes$style, 'color', '#a3a3a3')
				]),
			_List_fromArray(
				[
					$elm$html$Html$text('gampleman/elm-visualization · Scale + Shape + Axis primitives')
				]))
		]));
var $author$project$Main$chartDescription = function (active) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				...
				case 'AreaChartView':
					return _List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text('elm-visualization area chart: Scale.linear maps data → SVG coordinates. Shape.area + Shape.line generate SVG path strings. Axis.bottom/left render tick marks. Every element is under your control — useful when you need custom behavior beyond what higher-level libraries support.')
								]))
						]);
				case 'BarChartView':
					return _List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text('Scale.band maps categorical strings to pixel offsets with configurable padding. Each bar is a manual rect element. Rounded tops use a trick: a 4px rx/ry rect on top of a square-bottom rect.')
								]))
						]);
				default:
					return _List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text('Shape.pie computes start/end angles from values. Shape.arc renders each slice as a path. The hollow center (innerRadius = 60% of outer) is the donut shape. Central text is a PF pattern for showing the aggregate metric — total count, percentage used, etc.')
								]))
						]);
			}
		}());
...
var $author$project$AreaChart$networkData = _List_fromArray(
	[
		{inbound: 210, minuteOffset: 0, outbound: 145},
		{inbound: 280, minuteOffset: 15, outbound: 190},
		{inbound: 340, minuteOffset: 30, outbound: 220},
		{inbound: 310, minuteOffset: 45, outbound: 250},
		{inbound: 390, minuteOffset: 60, outbound: 275},
		{inbound: 355, minuteOffset: 75, outbound: 230},
		{inbound: 420, minuteOffset: 90, outbound: 300}
	]);
var $author$project$AreaChart$padding = {bottom: 48, left: 56, right: 24, top: 20};
var $author$project$AreaChart$width = 600;
var $author$project$AreaChart$innerWidth = ($author$project$AreaChart$width - $author$project$AreaChart$padding.left) - $author$project$AreaChart$padding.right;
var $author$project$Scale$Scale = function (a) {
	return {$: 'Scale', a: a};
};
...
var $author$project$Scale$Continuous$scaleWithTransform = F4(
	function (transform, untransform, range_, domain_) {
		return {
			convert: A2($author$project$Scale$Continuous$convertTransform, transform, $author$project$Interpolation$float),
			domain: domain_,
			invert: A2($author$project$Scale$Continuous$invertTransform, transform, untransform),
			nice: $author$project$Scale$Continuous$nice,
			range: range_,
			rangeExtent: F2(
				function (_v0, r) {
					return r;
				}),
			tickFormat: $author$project$Scale$Continuous$tickFormat,
			ticks: $author$project$Scale$Continuous$ticks
		};
	});
...
var $author$project$Scale$linear = F2(
	function (range_, domain_) {
		return $author$project$Scale$Scale(
			A2($author$project$Scale$Continuous$linear, range_, domain_));
	});
var $author$project$AreaChart$xScale = A2(
	$author$project$Scale$linear,
	_Utils_Tuple2(0, $author$project$AreaChart$innerWidth),
	_Utils_Tuple2(0, 90));
var $author$project$AreaChart$height = 300;
var $author$project$AreaChart$innerHeight = ($author$project$AreaChart$height - $author$project$AreaChart$padding.top) - $author$project$AreaChart$padding.bottom;
var $author$project$AreaChart$yScale = A2(
	$author$project$Scale$linear,
	_Utils_Tuple2($author$project$AreaChart$innerHeight, 0),
	_Utils_Tuple2(0, 500));
var $author$project$AreaChart$areaPath = function (accessor) {
	return A2(
		...
		A2(
			...
			function (d) {
				return $elm$core$Maybe$Just(
					_Utils_Tuple2(
						_Utils_Tuple2(
							A2($author$project$Scale$convert, $author$project$AreaChart$xScale, d.minuteOffset),
							A2($author$project$Scale$convert, $author$project$AreaChart$yScale, 0)),
						_Utils_Tuple2(
							A2($author$project$Scale$convert, $author$project$AreaChart$xScale, d.minuteOffset),
							A2(
								$author$project$Scale$convert,
								$author$project$AreaChart$yScale,
								accessor(d)))));
			},
			$author$project$AreaChart$networkData));
};
var $author$project$PF$Colors$blue300 = '#0066cc';
var $author$project$PF$Colors$blue500 = '#003366';
...
var $author$project$PF$Colors$gridLine = '#c7c7c7';
...
var $author$project$AreaChart$gridLines = function (count) {
	return A2(
		$elm$core$List$map,
		function (i) {
			var yPos = (i * $author$project$AreaChart$innerHeight) / count;
			return A2(
				$elm$svg$Svg$line,
				_List_fromArray(
					[
						...
						$elm$core$String$fromFloat($author$project$AreaChart$innerWidth)),
						$elm$svg$Svg$Attributes$y2(
						$elm$core$String$fromFloat(yPos)),
						$elm$svg$Svg$Attributes$stroke($author$project$PF$Colors$gridLine),
...
var $author$project$AreaChart$hexCharToInt = function (c) {
	switch (c.valueOf()) {
		case '0':
			return 0;
		case '1':
...
var $author$project$AreaChart$hexPairToInt = F2(
	function (hi, lo) {
		return ($author$project$AreaChart$hexCharToInt(hi) * 16) + $author$project$AreaChart$hexCharToInt(lo);
	});
var $elm$core$String$foldr = _String_foldr;
var $elm$core$String$toList = function (string) {
	return A3($elm$core$String$foldr, $elm$core$List$cons, _List_Nil, string);
};
var $author$project$AreaChart$hexWithAlpha = F2(
	function (hex, alpha) {
		...
		if ((((((_v0.b && _v0.b.b) && _v0.b.b.b) && _v0.b.b.b.b) && _v0.b.b.b.b.b) && _v0.b.b.b.b.b.b) && (!_v0.b.b.b.b.b.b.b)) {
			...
			var _v5 = _v4.b;
			var b2 = _v5.a;
			var r = A2($author$project$AreaChart$hexPairToInt, r1, r2);
			var g = A2($author$project$AreaChart$hexPairToInt, g1, g2);
			var b = A2($author$project$AreaChart$hexPairToInt, b1, b2);
			return 'rgba(' + ($elm$core$String$fromInt(r) + (',' + ($elm$core$String$fromInt(g) + (',' + ($elm$core$String$fromInt(b) + (',' + ($elm$core$String$fromFloat(alpha) + ')')))))));
		} else {
			return hex;
		}
	});
...
var $author$project$AreaChart$linePath = function (accessor) {
	return A2(
...
var $author$project$PF$Colors$labelText = '#383838';
var $author$project$AreaChart$styledAxis = function (axisEl) {
	return A2(
		$elm$svg$Svg$g,
		_List_fromArray(
			[
				$elm$svg$Svg$Attributes$style('color: ' + ($author$project$PF$Colors$labelText + ('; fill: ' + ($author$project$PF$Colors$labelText + ';')))),
				$elm$svg$Svg$Attributes$class('pf-axis')
			]),
		_List_fromArray(
			[axisEl]));
};
	...
	function (attrs, scale) {
		...
		return A3(
				...
				}),
			{
				...
				tickFormat: A2($author$project$Scale$tickFormat, scale, opts.tickCount),
				tickPadding: opts.tickPadding,
				tickSizeInner: opts.tickSizeInner,
				tickSizeOuter: opts.tickSizeOuter,
				ticks: A2($author$project$Scale$ticks, scale, opts.tickCount)
			},
			postList);
	});
...
var $author$project$AreaChart$xAxis = A2(
	$author$project$Axis$bottom,
	_List_fromArray(
		[
			$author$project$Axis$tickCount(7),
			$author$project$Axis$tickFormat(
			function (v) {
				return $elm$core$String$fromFloat(v) + 'm';
			})
		]),
	$author$project$AreaChart$xScale);
...
var $author$project$AreaChart$yAxis = A2(
	...
	_List_fromArray(
		[
			...
			$author$project$Axis$tickFormat(
			function (v) {
				return $elm$core$String$fromFloat(v) + ' Mbps';
			})
		]),
	$author$project$AreaChart$yScale);
var $author$project$AreaChart$chart = A2(
	$elm$svg$Svg$svg,
	_List_fromArray(
		[
			$elm$svg$Svg$Attributes$width(
			$elm$core$String$fromFloat($author$project$AreaChart$width)),
			$elm$svg$Svg$Attributes$height(
			$elm$core$String$fromFloat($author$project$AreaChart$height)),
			$elm$svg$Svg$Attributes$style('font-family: \'Red Hat Text\', Helvetica, Arial, sans-serif; font-size: 12px; overflow: visible;')
		]),
	_List_fromArray(
		[
			A2(
			...
			_List_fromArray(
				[
					$elm$svg$Svg$Attributes$transform(
					'translate(' + ($elm$core$String$fromFloat($author$project$AreaChart$padding.left) + (',' + ($elm$core$String$fromFloat($author$project$AreaChart$padding.top) + ')'))))
				]),
			_List_fromArray(
				[
					A2(
					$elm$svg$Svg$g,
					_List_Nil,
					$author$project$AreaChart$gridLines(5)),
					A2(
					$folkertdev$one_true_path_experiment$Path$element,
					$author$project$AreaChart$areaPath(
						function ($) {
							return $.inbound;
						}),
					_List_fromArray(
						[
							...
							A2($author$project$AreaChart$hexWithAlpha, $author$project$PF$Colors$blue300, 0.15)),
					...
					_List_fromArray(
						[
							$elm$svg$Svg$Attributes$fill(
							A2($author$project$AreaChart$hexWithAlpha, $author$project$PF$Colors$blue500, 0.15)),
							$elm$svg$Svg$Attributes$stroke('none')
						])),
					A2(
					$folkertdev$one_true_path_experiment$Path$element,
					$author$project$AreaChart$linePath(
						function ($) {
							return $.inbound;
						}),
					_List_fromArray(
						[
							$elm$svg$Svg$Attributes$fill('none'),
							$elm$svg$Svg$Attributes$stroke($author$project$PF$Colors$blue300),
					...
					_List_fromArray(
						[
							$elm$svg$Svg$Attributes$fill('none'),
							$elm$svg$Svg$Attributes$stroke($author$project$PF$Colors$blue500),
							$elm$svg$Svg$Attributes$strokeWidth('2')
						])),
					A2(
					$elm$svg$Svg$g,
					_List_fromArray(
						[
							$elm$svg$Svg$Attributes$transform(
							'translate(0,' + ($elm$core$String$fromFloat($author$project$AreaChart$innerHeight) + ')'))
						]),
					_List_fromArray(
						[
							$author$project$AreaChart$styledAxis($author$project$AreaChart$xAxis)
						])),
					$author$project$AreaChart$styledAxis($author$project$AreaChart$yAxis)
				]))
		]));
var $elm$html$Html$h2 = _VirtualDom_node('h2');
var $author$project$AreaChart$legendItem = F2(
...
var $author$project$AreaChart$legend = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
			A2($elm$html$Html$Attributes$style, 'display', 'flex'),
			...
			'padding-left',
			$elm$core$String$fromFloat($author$project$AreaChart$padding.left) + 'px'),
			A2($elm$html$Html$Attributes$style, 'font-size', '12px'),
			A2($elm$html$Html$Attributes$style, 'color', $author$project$PF$Colors$labelText)
		]),
	_List_fromArray(
		[
			A2($author$project$AreaChart$legendItem, 'Inbound', $author$project$PF$Colors$blue300),
			A2($author$project$AreaChart$legendItem, 'Outbound', $author$project$PF$Colors$blue500)
		]));
var $author$project$AreaChart$view = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
			A2($elm$html$Html$Attributes$style, 'font-family', '\'Red Hat Text\', Helvetica, Arial, sans-serif'),
		...
		[
			...
			_List_fromArray(
				[
					...
					A2($elm$html$Html$Attributes$style, 'color', '#1f1f1f')
				]),
			_List_fromArray(
				[
					$elm$html$Html$text('Network Throughput')
				])),
			A2(
			$elm$html$Html$p,
			_List_fromArray(
			...
			_List_fromArray(
				[
					$elm$html$Html$text('compute-node-01 · last 90 minutes')
				])),
			$author$project$AreaChart$chart,
			$author$project$AreaChart$legend
		]));
var $author$project$Scale$bandwidth = function (_v0) {
	var scale = _v0.a;
	return scale.bandwidth;
};
var $author$project$BarChart$height = 300;
var $author$project$BarChart$padding = {bottom: 48, left: 60, right: 24, top: 20};
var $author$project$BarChart$innerHeight = ($author$project$BarChart$height - $author$project$BarChart$padding.top) - $author$project$BarChart$padding.bottom;
...
var $author$project$PF$Colors$multiOrdered = _List_fromArray(
	['#0066cc', '#63993d', '#37a3a3', '#dca614', '#f5921b', '#4394e5', '#87bb62', '#63bdbd', '#ffcc17', '#f8ae54']);
var $author$project$PF$Colors$primary = $author$project$PF$Colors$blue300;
var $author$project$BarChart$pfColor = function (idx) {
	return A2(
		$elm$core$Maybe$withDefault,
		$author$project$PF$Colors$primary,
		$elm$core$List$head(
			A2(
				$elm$core$List$drop,
				A2(
					$elm$core$Basics$modBy,
					$elm$core$List$length($author$project$PF$Colors$multiOrdered),
					idx),
				$author$project$PF$Colors$multiOrdered)));
};
...
var $author$project$Scale$band = F3(
	function (config, range_, domain_) {
		return $author$project$Scale$Scale(
			{
				bandwidth: A3($author$project$Scale$Band$bandwidth, config, domain_, range_),
				convert: $author$project$Scale$Band$convert(config),
				domain: domain_,
				range: range_
			});
	});
var $author$project$BarChart$width = 580;
var $author$project$BarChart$innerWidth = ($author$project$BarChart$width - $author$project$BarChart$padding.left) - $author$project$BarChart$padding.right;
var $author$project$BarChart$regions = _List_fromArray(
	[
		{count: 142, name: 'us-east-1'},
		{count: 98, name: 'us-west-2'},
		{count: 187, name: 'eu-central'},
		{count: 74, name: 'eu-west-1'},
		{count: 115, name: 'ap-south'},
		{count: 53, name: 'ap-east'}
	]);
var $author$project$BarChart$xScale = A3(
	$author$project$Scale$band,
	{align: 0.5, paddingInner: 0.2, paddingOuter: 0.15},
	_Utils_Tuple2(0, $author$project$BarChart$innerWidth),
	A2(
		$elm$core$List$map,
		function ($) {
			return $.name;
		},
		$author$project$BarChart$regions));
var $author$project$BarChart$yScale = A2(
	$author$project$Scale$linear,
	_Utils_Tuple2($author$project$BarChart$innerHeight, 0),
	_Utils_Tuple2(0, 220));
var $author$project$BarChart$bar = F2(
	function (idx, region) {
		var color = $author$project$BarChart$pfColor(idx);
		var barY = A2($author$project$Scale$convert, $author$project$BarChart$yScale, region.count);
		var barX = A2($author$project$Scale$convert, $author$project$BarChart$xScale, region.name);
		var barWidth = $author$project$Scale$bandwidth($author$project$BarChart$xScale);
		var barHeight = $author$project$BarChart$innerHeight - A2($author$project$Scale$convert, $author$project$BarChart$yScale, region.count);
		return A2(
			$elm$svg$Svg$g,
			_List_Nil,
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							...
							$elm$svg$Svg$Attributes$height(
							$elm$core$String$fromFloat(barHeight - 2)),
...
var $author$project$BarChart$gridLines = function (count) {
	return A2(
		$elm$core$List$map,
		function (i) {
			var yPos = (i * $author$project$BarChart$innerHeight) / count;
			return A2(
				$elm$svg$Svg$line,
				_List_fromArray(
					[
						...
						$elm$core$String$fromFloat(yPos)),
						$elm$svg$Svg$Attributes$x2(
						$elm$core$String$fromFloat($author$project$BarChart$innerWidth)),
						$elm$svg$Svg$Attributes$y2(
						$elm$core$String$fromFloat(yPos)),
						$elm$svg$Svg$Attributes$stroke($author$project$PF$Colors$gridLine),
						$elm$svg$Svg$Attributes$strokeWidth('1')
...
var $author$project$BarChart$styledAxis = function (axisEl) {
	return A2(
		$elm$svg$Svg$g,
		_List_fromArray(
			[
...
var $author$project$BarChart$xAxis = A2(
	$author$project$Axis$bottom,
	_List_Nil,
	A2($author$project$Scale$toRenderable, $elm$core$Basics$identity, $author$project$BarChart$xScale));
...
var $author$project$BarChart$chart = A2(
	$elm$svg$Svg$svg,
	_List_fromArray(
		[
			$elm$svg$Svg$Attributes$width(
			$elm$core$String$fromFloat($author$project$BarChart$width)),
			$elm$svg$Svg$Attributes$height(
			$elm$core$String$fromFloat($author$project$BarChart$height)),
			$elm$svg$Svg$Attributes$style('font-family: \'Red Hat Text\', Helvetica, Arial, sans-serif; font-size: 12px; overflow: visible;')
		]),
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$svg$Svg$Attributes$transform(
					'translate(' + ($elm$core$String$fromFloat($author$project$BarChart$padding.left) + (',' + ($elm$core$String$fromFloat($author$project$BarChart$padding.top) + ')'))))
				]),
			_List_fromArray(
				[
					A2(
					$elm$svg$Svg$g,
					_List_Nil,
					$author$project$BarChart$gridLines(5)),
					A2(
					$elm$svg$Svg$g,
					_List_Nil,
					A2($elm$core$List$indexedMap, $author$project$BarChart$bar, $author$project$BarChart$regions)),
					A2(
					$elm$svg$Svg$g,
					_List_fromArray(
						[
							$elm$svg$Svg$Attributes$transform(
							'translate(0,' + ($elm$core$String$fromFloat($author$project$BarChart$innerHeight) + ')'))
						]),
					_List_fromArray(
						[
							$author$project$BarChart$styledAxis($author$project$BarChart$xAxis)
						])),
					$author$project$BarChart$styledAxis($author$project$BarChart$yAxis)
				]))
		]));
var $author$project$BarChart$view = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
			A2($elm$html$Html$Attributes$style, 'font-family', '\'Red Hat Text\', Helvetica, Arial, sans-serif'),
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$html$Html$text('Active Instances by Region')
				])),
			A2(
			$elm$html$Html$p,
			_List_fromArray(
			...
			_List_fromArray(
				[
					$elm$html$Html$text('Virtual machine count · all regions')
				])),
			$author$project$BarChart$chart
		]));
var $elm$svg$Svg$Attributes$dominantBaseline = _VirtualDom_attribute('dominant-baseline');
var $elm$svg$Svg$Attributes$fontWeight = _VirtualDom_attribute('font-weight');
var $author$project$DonutChart$allocationData = _List_fromArray(
	[
...
var $author$project$DonutChart$totalVcpus = A3(
	$elm$core$List$foldl,
	F2(
		function (s, acc) {
			return acc + s.vcpus;
	...
	0,
	$author$project$DonutChart$allocationData);
var $author$project$DonutChart$centerText = A2(
	$elm$svg$Svg$g,
	_List_Nil,
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$svg$Svg$text(
					$elm$core$String$fromFloat($author$project$DonutChart$totalVcpus))
				])),
			A2(
			$elm$svg$Svg$text_,
			_List_fromArray(
				[
					...
					$elm$svg$Svg$Attributes$fill('#707070')
				]),
			_List_fromArray(
				[
					$elm$svg$Svg$text('vCPUs allocated')
				]))
		]));
var $author$project$DonutChart$size = 280;
var $elm$core$Basics$compare = _Utils_compare;
var $elm$core$Basics$pi = _Basics_pi;
var $author$project$Shape$defaultPieConfig = {cornerRadius: 0, endAngle: 2 * $elm$core$Basics$pi, innerRadius: 0, outerRadius: 100, padAngle: 0, padRadius: 0, sortingFn: $elm$core$Basics$compare, startAngle: 0, valueFn: $elm$core$Basics$identity};
var $author$project$DonutChart$radius = (A2($elm$core$Basics$min, $author$project$DonutChart$size, $author$project$DonutChart$size) / 2) - 10;
var $author$project$DonutChart$innerRadius = $author$project$DonutChart$radius * 0.6;
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									{
									arcFlag: $author$project$Shape$Pie$boolToArc(
...
var $author$project$Shape$Pie$arc = function (arcData) {
	...
	if (_Utils_cmp(r1, $author$project$Shape$Pie$epsilon) < 1) {
		...
		if (_Utils_cmp(da, (2 * $elm$core$Basics$pi) - $author$project$Shape$Pie$epsilon) > 0) {
			...
			var rp = (_Utils_cmp(ap, $author$project$Shape$Pie$epsilon) > 0) ? ((arcData.padRadius > 0) ? arcData.padRadius : $elm$core$Basics$sqrt(
...
var $author$project$DonutChart$pfColor = function (idx) {
	return A2(
		$elm$core$Maybe$withDefault,
		$author$project$PF$Colors$primary,
		$elm$core$List$head(
...
var $author$project$DonutChart$renderSlice = F2(
	function (idx, arc) {
		return A2(
			$folkertdev$one_true_path_experiment$Path$element,
			$author$project$Shape$arc(arc),
			_List_fromArray(
				[
					$elm$svg$Svg$Attributes$fill(
					$author$project$DonutChart$pfColor(idx)),
					$elm$svg$Svg$Attributes$stroke('#ffffff'),
					$elm$svg$Svg$Attributes$strokeWidth('2')
				]));
	});
var $author$project$DonutChart$slices = function () {
	var values = A2(
		$elm$core$List$map,
		function ($) {
			return $.vcpus;
		...
		$author$project$DonutChart$allocationData);
	var pieConfig = _Utils_update(
		$author$project$Shape$defaultPieConfig,
		{
			innerRadius: $author$project$DonutChart$innerRadius,
			outerRadius: $author$project$DonutChart$radius,
			padAngle: 0.02,
			sortingFn: F2(
				function (_v0, _v1) {
	...
	var arcs = A2($author$project$Shape$pie, pieConfig, values);
	return A2($elm$core$List$indexedMap, $author$project$DonutChart$renderSlice, arcs);
}();
var $author$project$DonutChart$chart = A2(
	$elm$svg$Svg$svg,
	_List_fromArray(
		[
			$elm$svg$Svg$Attributes$width(
			$elm$core$String$fromFloat($author$project$DonutChart$size)),
			$elm$svg$Svg$Attributes$height(
			$elm$core$String$fromFloat($author$project$DonutChart$size)),
			$elm$svg$Svg$Attributes$style('font-family: \'Red Hat Text\', Helvetica, Arial, sans-serif; overflow: visible;')
		]),
	_List_fromArray(
		[
			A2(
			...
			_List_fromArray(
				[
					$elm$svg$Svg$Attributes$transform(
					'translate(' + ($elm$core$String$fromFloat($author$project$DonutChart$size / 2) + (',' + ($elm$core$String$fromFloat($author$project$DonutChart$size / 2) + ')'))))
				]),
			_Utils_ap(
				$author$project$DonutChart$slices,
				_List_fromArray(
					[$author$project$DonutChart$centerText])))
		]));
var $author$project$DonutChart$legendRow = F2(
				...
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text(segment.label)
								])),
							A2(
							$elm$html$Html$div,
							_List_fromArray(
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									$elm$core$String$fromFloat(segment.vcpus) + ' vCPUs')
								]))
						]))
				]));
	});
var $author$project$DonutChart$legend = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
			A2($elm$html$Html$Attributes$style, 'font-size', '13px'),
	...
	A2($elm$core$List$indexedMap, $author$project$DonutChart$legendRow, $author$project$DonutChart$allocationData));
var $author$project$DonutChart$view = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
			A2($elm$html$Html$Attributes$style, 'font-family', '\'Red Hat Text\', Helvetica, Arial, sans-serif'),
	...
	_List_fromArray(
		[$author$project$DonutChart$chart, $author$project$DonutChart$legend]));
			...
			[
				...
				_List_fromArray(
					[
						...
						A2($elm$html$Html$Attributes$style, 'border', '1px solid #c7c7c7'),
				...
				_List_fromArray(
					[
						...
						switch (_v0.$) {
							case 'AreaChartView':
								return $author$project$AreaChart$view;
							case 'BarChartView':
								return $author$project$BarChart$view;
							default:
								return $author$project$DonutChart$view;
						}
					}()
					])),
				$author$project$Main$chartDescription(model.active)
			]));
};
var $author$project$Main$BarChartView = {$: 'BarChartView'};
var $author$project$Main$DonutChartView = {$: 'DonutChartView'};
var $author$project$Main$ShowChart = function (a) {
	return {$: 'ShowChart', a: a};
};
...
var $author$project$Main$navTab = F3(
	function (label, target, active) {
		var isActive = _Utils_eq(target, active);
		var textColor = isActive ? '#0066cc' : '#383838';
		var borderColor = isActive ? '#0066cc' : 'transparent';
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
					A2($elm$html$Html$Attributes$style, 'padding', '12px 20px'),
					A2($elm$html$Html$Attributes$style, 'border-bottom', '3px solid ' + borderColor),
					A2($elm$html$Html$Attributes$style, 'color', textColor),
					A2($elm$html$Html$Attributes$style, 'font-size', '14px'),
					A2($elm$html$Html$Attributes$style, 'cursor', 'pointer'),
					A2($elm$html$Html$Attributes$style, 'user-select', 'none'),
					$elm$html$Html$Events$onClick(
					$author$project$Main$ShowChart(target))
				]),
			_List_fromArray(
				[
					$elm$html$Html$text(label)
...
var $author$project$Main$nav = function (active) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				A2($elm$html$Html$Attributes$style, 'background', '#ffffff'),
				A2($elm$html$Html$Attributes$style, 'border-bottom', '1px solid #c7c7c7'),
				A2($elm$html$Html$Attributes$style, 'padding', '0 32px'),
				A2($elm$html$Html$Attributes$style, 'display', 'flex'),
				A2($elm$html$Html$Attributes$style, 'gap', '0')
			]),
		_List_fromArray(
			[
				A3($author$project$Main$navTab, 'Area Chart', $author$project$Main$AreaChartView, active),
				A3($author$project$Main$navTab, 'Bar Chart', $author$project$Main$BarChartView, active),
				A3($author$project$Main$navTab, 'Donut Chart', $author$project$Main$DonutChartView, active)
			]));
};
var $author$project$Main$view = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				A2($elm$html$Html$Attributes$style, 'font-family', '\'Red Hat Text\', Helvetica, Arial, sans-serif'),
				A2($elm$html$Html$Attributes$style, 'background', '#f2f2f2'),
		...
		_List_fromArray(
			[
				...
				$author$project$Main$nav(model.active),
